...
## Delegated (called from another skill, e.g. `pr-check` or `release-check`)
...
- the category label the caller's own checklist uses for this check (e.g.
  `pr-check`'s "Test coverage gaps")

Don't print a `PASS`/`FAIL`/`N/A` verdict yourself when delegated — the
caller owns that decision (for example, `pr-check` reports `N/A` itself
for `changelog-check` when Release Drafter is present, rather than the
sub-skill deciding that).
